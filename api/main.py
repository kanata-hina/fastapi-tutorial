...
async def createTask(id: int, task: Task):
    ...
    conn = get_connection()
    cur = conn.cursor()
    cur.execute("INSERT INTO task(user_id, title, description) VALUES(" +
                str(id) + ", '" + task.title + "', '" + task.description + "')")
    conn.commit()
...
async def getAllTasks(id: int, task_id: int):
    conn = get_connection()
    cur = conn.cursor()
    cur.execute('SELECT * FROM task where user_id = ' +
                str(id) + ' and id = ' + str(task_id))
    rows = cur.fetchall()
    if len(rows) == 0:
        raise HTTPException(status_code=500, detail="task id not found")
    print(rows)
...
async def getAllTasks(id: int, task_id: int, task: Task):
    conn = get_connection()
    cur = conn.cursor()
    cur.execute("UPDATE task SET title = '" + task.title + "', description = '" +
                task.description + "' where id = " + str(task_id) + " and user_id = " + str(id))
    conn.commit()
    cur.execute('SELECT * FROM task where user_id = ' +
                str(id) + ' and id = ' + str(task_id))
    rows = cur.fetchall()
